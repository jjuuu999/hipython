...
ON 사원.부서번호 = 부서.부서번호;
# WHERE 부서.부서번호 IS NULL >부서번호 없는 사원만 보여주려면,, 근데 사원.부서번호는 NULL값이 아닌 공백값이기 때문에 부서.부서번호 IS NULL로 조건 해야 함.
...
#서브쿼리 >> 이렇게 서브쿼리를 사용하는 이유가 이게 좀 더 직관적임. 비교(앞에 AS A AS B 이거 두개)
SELECT 고객번호
, 고객회사명
, 담당자명
, 마일리지
...
/* 담당자명, 고객회사명, 마일리지, 도시, 해당 도시의 평균마일리지를 보이시오. 
그리고 고객이 위치하는 도시의 평균마일리지와 각 고객의 마일리지 간의 차이도 
함께 보이시오. */
SELECT 담당자명
, 고객회사명
, 마일리지
...
#고객번호, 담당자명과 고객의 최종 주문일을 보이시오
SELECT 고객번호
, 담당자명
, (SELECT MAX(주문일)
FROM 주문
WHERE 주문.고객번호 = 고객.주문번호
) AS 최종주문일
FROM 고객;
